...
制作屏幕字时，必须使用上文中对齐帧的方式按帧对齐画面，不能有一帧偏差。
...
* 修改颜色:  
  Aegisub 的修改颜色窗口中，点吸管按钮即可在视频中取色，如果有噪点等导致每个像素颜色偏差过大，可以缩小视频，即可**取到平均色**。
...
* 需要蒙版时，使用裁切工具自定义蒙蔽，详见 `\clip` `\iclip` 用法。
...
* 需要**淡入淡出**时使用 `\fad`，需要自定义其透明度，使用 `\fade`，当然也可以使用 `\t` 代替。  
